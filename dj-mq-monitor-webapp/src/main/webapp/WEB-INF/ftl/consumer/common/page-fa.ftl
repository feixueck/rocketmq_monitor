...
<#macro page title="大街网_年轻人专属的社交招聘平台！" assets=[] static={} useheader="default" usefooter="default" currentChannel="NONE" bodyclass="none" menu="0" description="" keywords="" flash=[] crossdomains=[] footerassets=[] nofollow="0" viewport="0" metaExtend=[] linkExtend=[]>
<!DOCTYPE html>
...
<head>
    ...
    <#if metaExtend?? &&metaExtend?size gt 0>
        <#list metaExtend as metaItem>
            <#if metaItem?is_hash>
                <meta <#list metaItem?keys as key><#if metaItem[key]??>${key}="${metaItem[key]}"</#if> </#list>/></#if>
        </#list>
    </#if>
    <#if linkExtend?? &&linkExtend?size gt 0>
        <#list linkExtend as linkItem>
            <#if linkItem?is_hash>
                <link <#list linkItem?keys as key><#if linkItem[key]??>${key}="${linkItem[key]}"</#if> </#list>/></#if>
        </#list>
    </#if>
    ...
    <script id="djnew" type="text/javascript"
            src="${AssetsUrl("${WrapperAssetsWithVersion('/static/libs/street/street.js')}")}"></script>
    ...
    <script type="text/javascript">
        $.domains = {
            ...
            <#if (isInBlack?? && isInBlack==true) || (isBusinessCorp?? && isBusinessCorp==true)>
      	   	var dontShowFloatRegDialog = true;
            </#if>
    </script>
</head>
...
<script type="text/javascript">var _body_start = new Date();</script>
    <#if crossdomains?size!=0>
        ...
        <iframe src="http://${configs['domains.'+domain]}/crossdomain/proxy.html<#if currentDate??>?t=${currentDate.getTime()}</#if>"
                id="${domain}-ajax-frame" style="display:none;"></iframe>
        </#list>
    <script type="text/javascript">
        ...
        <#else>
            <#if useheader = 'default' && ChannelEnum[realChannel]?? && ChannelEnum[realChannel].navClass!=''>
                ...
                <#--每个频道的具体内容在header.ftl中控制-->
                    <#if ChannelEnum[realChannel].description?length gt 0>
                        <h2>${ChannelEnum[realChannel].description}<#if (ChannelEnum[realChannel].enName)??>
                            <em>${ChannelEnum[realChannel].enName}</em></#if></h2><#else><h2
                            style="display:none;"></h2></#if>
                </div>
            </#if>
            <div class="dj-content-shadow">
                <#nested/>
...
<script type="text/javascript">
    ...
    || !(request.requestURL.toString()?contains(configs['domains.www']+'/account/login') || request.requestURL.toString()?contains(configs['domains.www']+'/account/reg'))
       ...
       <#include 'account/floataccount-pop-sea.ftl'/>
   <#else >
...
<#-- current_corp?? && !(CorpBusinessOrder??) 有订单的公司不出现此浮层
    ...
    >
        <#include 'account/account-reg-fa.ftl'/>
