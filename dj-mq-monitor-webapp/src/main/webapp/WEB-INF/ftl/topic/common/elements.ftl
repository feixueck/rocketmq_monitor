...
<#macro cityInJson hotCityList=['310000','110000','440100','440300','330100','420100','510100','320100','120000','210100','610100','230100','500000','370200','370100','210200','320500']>
<script type="text/javascript">
    var DJ_DATA_CITY = {
        "hot" : [
        <#list hotCityList as hotCity>
            <#if (dictMap.DICT_CITY.map.get(hotCity?number?int).name)??>
            ["${dictMap.DICT_CITY.map.get(hotCity?number?int).name}", ${hotCity?number?int}]<#if hotCity_has_next>,</#if>
            </#if>
        </#list>
        ],
        "normal": [
        <#assign isFirstCity = true/>
        <#list dictMap["DICT_CITY"].data as city>
            <#if city.parentId gt 0>
                <#break />
            </#if>
            <#if !isFirstCity>,<#else><#assign isFirstCity = false/></#if>
            {
                "main" : ["${city.name}",${city.id}],
                "sub" : [
                    <#if city.children?? && city.children?size gt 0>
                    <#list city.children as child>
                        ["${child.name}",${child.id}]<#if child_has_next>,</#if>
                    </#list>
                    </#if>
                ]
            }
