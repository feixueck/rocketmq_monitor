...
<div id="dj-dialog-reg" style="display:none;">
    <div class="nt-info">
        ...
        <div class="nt-con">
            <p>Hi，现在就使用名企绿色通道：</p>
            <h4 style="padding-right: 80px;`" id="title"></h4>
        </div>
        <b></b>
    </div>
...
<script type="text/javascript">
    function pop_reg(redir,defaultPop,popRegFloat) {
        ...
        $.dialog('#dj-dialog-reg', {
            ...
            onabort:function(){
                typeof click_b =='function' && click_b("clubtest8_close");
            }
        });
        typeof click_b == 'function' && click_b("clubtest8");
        ...
        $('.dj-dialog-reg-renren a').attr('href', 'http://${configs['domains.www']}/account/rr/login?d.from=clubtest8&redir='+encodeURIComponent(redir));
        $('.dj-dialog-reg-kaixin a').attr('href', 'http://${configs['domains.www']}/account/kx/login?d.from=clubtest8&redir='+encodeURIComponent(redir));
        $('.dj-dialog-reg-qq a').attr('href', 'http://${configs['domains.www']}/account/qq/login?d.from=clubtest8&redir='+encodeURIComponent(redir));
        $('.dj-dialog-reg-sina a').attr('href', 'http://${configs['domains.www']}/account/sina/login?d.from=clubtest8&d.cb=r0&redir='+encodeURIComponent(redir));
        $('#dj-dialog-reg-form').attr('action', 'http://${configs['domains.www']}/account/regsubmit?d.from=clubtest8&d.template=club_reg&redir='+encodeURIComponent(redir));
        $('#login').attr('href', 'http://${configs['domains.www']}/account/login?d.from=clubtest8&redir='+encodeURIComponent(redir));
            ...
            $.formValidate.submit(strRegSubmitClass, function(r){
                if(r){
                    if($.cookie('DJ_RF') != null){
                        var cookie = $.cookie('DJ_RF').replace('"', '').replace('"', '') + "?clubreg";
    ...
    function djDialogRegEmailCheck(r, obj){
        var errorinfo='该邮箱已注册，请<a href="http://${configs['domains.www']}/account/logout?d.from=clubtest8">直接登录</a>';
        if(!r){
            var $this = $(this);
    ...
    (function(){
        var title = ["提前了解<strong>${(current_corp.name)!}</strong>面试要求，迅速提高求职成功率","简单设置，让<strong>${(current_corp.name)!'合适'}</strong>的工作机会主动联系你","鼠标轻点，<strong>${(current_corp.name)!'合适'}</strong>的工作自动跳到你碗里去",
            "把握机会，让<strong>${(current_corp.name)!'合适'}</strong>的工作离自己更近一步"];
